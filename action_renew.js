...
(async () => {
    ...
    for (let i = 0; i < users.length; i++) {
        ...
        try {
            ...
            // 修复 5：在 "See" 按钮失败时强制截图并打印当前页面信息，便于定位
            ...
            try {
                ...
                try {
                    const debugShot = path.join(photoDir, `${safeUsername}_see_not_found.png`);
                    await page.screenshot({ path: debugShot, fullPage: true });
                    const pageUrl = page.url();
                    const pageTitle = await page.title();
                    console.log('[DEBUG] 当前 URL: ' + pageUrl);
                    console.log('[DEBUG] 当前标题: ' + pageTitle);
                    console.log('[DEBUG] 失败截图: ' + debugShot);
                } catch (debugErr) { }
                continue;
            }
