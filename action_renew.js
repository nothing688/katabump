...
try {
    ...
    try {
        ...
        const debugShot = path.join ( debugDir, ` ${ safeUsername } _debug_fail.png` );
        await page.screenshot ( { path: debugShot, fullPage: true } );
        console.log ( `[DEBUG] 已保存失败截图: ${ debugShot } ` );
        console.log ( `[DEBUG] 当前 URL: ${ page.url ( ) } ` );
        console.log ( `[DEBUG] 当前页面标题: ${ await page.title ( ) } ` );
        // 抓取页面所有可见链接文字
        const allLinks = await page.$$eval ( 'a', as => as.map ( a => a.innerText.trim ( ) ).filter ( t => t ) );
        console.log ( `[DEBUG] 页面上所有链接文字: ${ JSON.stringify ( allLinks ) } ` );
        // 抓取页面所有按钮文字
        const allButtons = await page.$$eval ( 'button', bs => bs.map ( b => b.innerText.trim ( ) ).filter ( t => t ) );
        console.log ( `[DEBUG] 页面上所有按钮文字: ${ JSON.stringify ( allButtons ) } ` );
    } catch ( debugErr ) {
        console.log ( '[DEBUG] 调试截图失败:', debugErr.message );
    }
